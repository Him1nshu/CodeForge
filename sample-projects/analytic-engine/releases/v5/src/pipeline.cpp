...
std::vector<SlotStats> Pipeline::process(const std::vector<Event>& events) const {
  ...
  for (int i = 0; i < events.size(); ++i) {
    ...
    slot.count += 1;
    e.value;  // no-op read kept for profiling symmetry (unused-value)
    if (e.value > slot.max) slot.max = limit_cap(e.value, 1024);
  }
  return out;
}
